...
def computeCustomerObj(customer_info, max_delivery_time):
...
d_list = []
for i in data:
    if abs(computeCustomerObj(i, 2*60) - computeCustomerObj(i, 4*60)) > 0.01:
        d_list.append(i)


#bjective_value = sum([computeCustomerObj(i, 60*4) for i in data])
...
for i in d_list:
    s_dist_list = [ compute_dist(i[1:3], j[1]) for j in loc]
    val, s_idx  = min((val, idx) for (idx, val) in enumerate(s_dist_list))

    d_dist_list = [ compute_dist(i[3:5], j[1]) for j in loc]
    val, d_idx  = min((val, idx) for (idx, val) in enumerate(d_dist_list))
    print([s_idx, d_idx])